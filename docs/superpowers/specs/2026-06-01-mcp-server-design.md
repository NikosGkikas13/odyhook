...
### REST consistency (recommended, optional)
Surface additions 1 & 2 on the public REST API too — `GET /api/v1/events?sourceId=…&since=…&until=…` and a new `GET /api/v1/deliveries` — so the public API and MCP don't drift. Cheap, since the services are shared. Update `/openapi.json` accordingly.
...
7. **(Optional) REST consistency:** `/api/v1/events` filters + `/api/v1/deliveries`; update `/openapi.json`.
