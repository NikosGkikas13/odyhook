...
export function composeDestinationDisabledEmail(
  ...
  return {
    ...
    text: [
      ...
      "Events that were already queued at the time of auto-disable may appear",
      `as "destination paused" on your Events page and can be replayed after resuming.`,
      "New events arriving while the destination is disabled are not queued.",
      "",
      `Resume here: ${link}`,
    ].join("\n"),
  };
